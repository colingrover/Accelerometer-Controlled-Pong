...
void controlFromArduino () {
  ...
  if (!Float.isNaN(arduinoData.x)) {
    tempx = -map(arduinoData.x, -arduinoMaxAcceleration, arduinoMaxAcceleration, -accelerationScaleFactor, accelerationScaleFactor)*paddleSpeed; // Calculate possible change in x position from acceleration
  }
